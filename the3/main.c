 ...
 * Program starts in main function as you know,in here we are checking which 
 interrupt can be occur,There are 4 option  because of HW specification which are ADIF for ADC ,
 TMROIF,TMR1IF for timers interrupt and RBIF for RB button 4,5,6,7 bits on change any time interrupt

 * We configure ports before start the main function after that in any time  if come interrupt 
  program go on in the my_isr function. In here we checked which interrupt came.

 * After that,turn back the main function and go to related function after some flag is set.

 * These functions can be RB4pressed,tmr0interrupt,tmr1 interrupt,AdcCalculate.

 * In RB4pressed => we are checking guess==special_function  if so, we blinking with TMR1 for 500ms

 * if not the case, we give a hint (down arrow or up arrow).Morover,maybe instead of RB4 there are different pins may cause interrupt for RBIF which are RB5,RB6,RB7 so we also
 ...
 * IN TMR1interupt,we hold 5s in case of no correct answer in this time,
  and if correct answer comes in time (5s),we set timer1  for 500ms to blinking

 *In ADC_calcute, we have ADRESL,ADRESH.because of right justified ,ADRESL register hold last8bits which is less significant
 also,ADRESH hold first 2 bit which is most significant. I calculate total digital number in here and show this value in 7 segment display.

 *There are 2 while loop in main function one of them is provide us if program finish somehow 
 * again program will be started ,inside while loop is main loop this provide all operations actually handled with this part.  
...
int rB4pressed=0;     
int guess=0;        // after player pressed the RB4 ,adc_value will be assigned guess
...
int blink=0;   // this is used for turn of and turn on 7 segment display like a counter according to mod2 => 1 is turn on, 0 is turn off
int finishedCounter=0; //We hold this counter to finish game if finishedCounter=5 this means game finihed and it shoul started again.
int startAgain=0;
...
void tmr0interrupt(){
    ...
    /* I JUST ENABLE ADC enable bits which are  ADIE and GODONE to start ADC */
    ADIE=1;
...
void adcCalculate(){
/* We need to calculate binary value to convert decimal value which comes from voltage
 We use right justified mode in ADCON2 so ,we need gain last 2 bits from the ADRESH and 8bits(all)
ADRESL which compose binary value of the given voltage after that we need to calculate
decimal value of this binary value and then lastly,we need to map according to HW table and gain result value
...
void tmr1interrupt(){
    ...
    //this if block means game finish...
     if(finishedCounter==4) {
         startAgain=0;//restart counter if 0 which means game will start again.
         showAnswer=0;
     }
      ...
      //new value for TMR1  for 500ms
        ...
        // Game finish and we should show special number in  7 Segment display for 2 sec.
    if(blink%2==0){
        //SHOW RESULT TURN ON 
        LATH = 0b00001000 ; 
